defmodule Chatbot.InformationCollector do
  ...
  def terminate(:timeout, state) do
    ...
    GenServer.cast(
      state.leader,
      {:worker_dead, self(), state.user,
       gettext("Due to inactivity the conversation will be ended. It wont be saved.")}
    )
  ...
  @impl true
  def handle_cast(:stablished_skip, state) do
    keyboard = [
    ...
    TelegramWrapper.send_menu(
      keyboard,
      gettext("Do you want to add a review?"),
      state.user,
      state.key
    )
  ...
  @impl true
  def handle_cast(
        {:answer, %{"callback_query" => %{"data" => "NO"} = query, "update_id" => _}},
  ...
  @impl true
  def handle_cast(
        {:answer, %{"callback_query" => %{"data" => "YES"} = query, "update_id" => _}},
        %{data: %{review: nil}} = state
      ) do
    TelegramWrapper.answer_callback_query(state.key, query["id"])
    ...
    TelegramWrapper.update_menu(
      keyboard,
      gettext("Value the bot with the following stars:"),
      state.user,
  ...
  @impl true
  def handle_cast(
        {:answer, %{"callback_query" => %{"data" => stars} = query, "update_id" => _}},
        %{data: %{review: nil} = data} = state
      )
      when stars in ["1", "2", "3", "4", "5"] do
    TelegramWrapper.answer_callback_query(state.key, query["id"])

    TelegramWrapper.update_menu(
      [[%{text: "NO", callback_data: "NO"}]],
      gettext("Any suggestions or comments that you would like to write?"),
      state.user,
      query["message"]["message_id"],
      state.key
    )
  ...
  @impl true
  def handle_cast(
        {:answer, %{"callback_query" => query, "update_id" => _}},
        %{data: %{gender: nil} = data} = state
      ) do
    TelegramWrapper.answer_callback_query(state.key, query["id"])
  ...
  def handle_cast(
        {:answer, %{"callback_query" => query, "update_id" => _}},
        %{data: %{ca: nil} = data} = state
      ) do
    TelegramWrapper.answer_callback_query(state.key, query["id"])

    TelegramWrapper.update_menu(
      gettext("Describe what happened in detail."),
      state.user,
      query["message"]["message_id"],
      state.key
    )
  ...
  defp do_ask_ccaa(state, message_id) do
    keyboard = [
      [
        %{text: "GALICIA", callback_data: "GALIZA"},
        %{text: "CATALUÑA", callback_data: "CATALUNYA"}
      ],
      [
        %{text: "PAÍS VASCO", callback_data: "EUSKADI"},
        %{text: "MADRID", callback_data: "MADRID"}
      ],
      [
        %{text: "ANDALUCÍA", callback_data: "ANDALUCIA"},
        %{text: "ARAGON", callback_data: "ARAGON"}
      ],
      [
        %{text: "ASTURIAS", callback_data: "ASTURIAS"},
        %{text: "BALEARES", callback_data: "BALEARES"}
      ],
      [
        %{text: "CANARIAS", callback_data: "CANARIAS"},
        %{text: "CANTABRIA", callback_data: "CANTABRIA"}
      ],
      [
        %{text: "C. LA MANCHA", callback_data: "C. LA MANCHA"},
        %{text: "C. LEÓN", callback_data: "C. LEÓN"}
      ],
      [
        %{text: "COM. VALENCIANA", callback_data: "VALENCIA"},
        %{text: "EXTREMADURA", callback_data: "EXTREMADURA"}
      ],
    ...
    TelegramWrapper.update_menu(
      keyboard,
      gettext("In which CA did it happen?"),
      state.user,
      message_id,
      state.key
    )
    ...
    case GenServer.call(
      ...
      :not_created ->
        TelegramWrapper.send_message(
          state.key,
          state.user,
          gettext("Due to a server problem this information couldn't be saved.")
        )
      ...
      _ ->
        TelegramWrapper.send_message(
          state.key,
          state.user,
          gettext("This information has been saved successfully")
        )
    end
  end
