defmodule Chatbot.TelegramWrapper do
  require Logger
  alias Chatbot.NativeWrapper

  def send_message(key, chat_id, text) when is_binary(chat_id) do
    NativeWrapper.send_message(key, chat_id, text)
  end
  ...
  def answer_callback_query(_key, "http_callback") do
    NativeWrapper.answer_callback_query(nil, "http_callback")
  end
  def answer_callback_query(key, query_id) do
    Telegram.Api.request(key, "answerCallbackQuery", callback_query_id: query_id)
  end

  def delete_message(key, chat_id, message_id) when is_binary(chat_id) do
    NativeWrapper.delete_message(key, chat_id, message_id)
  end
  ...
  def update_menu(text, chat_id, message_id, key) when is_binary(chat_id) do
    NativeWrapper.update_menu(text, chat_id, message_id, key)
  end
  ...
  def update_menu(keyboard, text, chat_id, message_id, key) when is_binary(chat_id) do
    NativeWrapper.update_menu(keyboard, text, chat_id, message_id, key)
  end
  ...
  def send_menu(keyboard, message, chat_id, key) when is_binary(chat_id) do
    NativeWrapper.send_menu(keyboard, message, chat_id, key)
  end
  ...
  def send_image(image_path, chat_id, key) when is_binary(chat_id) do
    NativeWrapper.send_image(image_path, chat_id, key)
  end
